...
var messages = [
	"Bien Hecho!",
	"¿Cuantos Quedan?",
	"¡Esta resbaloso por aqui!",
	"¿Que pasara si me caigo?",
	"¡No Mires Abajo!",
	"Solo unos mas",
	"Pan comido",
]
...
func _physics_process(delta):
	...
	if (self.translation.y < -60):
		get_parent().get_node("CanvasLayer/Node2D/Label").display_self("¡Estamos cayendo!")
	...
	if Input.is_action_just_pressed("jump"):
		if jump_count < jump_max:
			...
			get_parent().get_node("CanvasLayer/Node2D/Label").display_self("¡No mas saltos!")
	...
	if (Input.is_action_just_pressed("turn")):
		if (Global.player_position == Vector3.ZERO):
			...
			get_parent().get_node("CanvasLayer/Node2D/Label").display_self("Punto de Teletranspotacion Fijado")
		else:
			self.translation = Global.player_position
			$Head.rotation_degrees = Global.player_rotation
			Global.player_position = Vector3.ZERO
			Global.player_rotation = null
			get_parent().get_node("CanvasLayer/Node2D/Label").display_self("Punto de teletransportacion activado")
