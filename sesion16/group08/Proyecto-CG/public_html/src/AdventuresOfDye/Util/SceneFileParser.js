...
SceneFileParser.prototype.parsePlatform2 = function (texture, normal, lightSet) {
    var elm = this._getElm("Platform2");
    var i, j, x, y, v, r, p;
    var allPlatforms2 = [];
    for (i = 0; i < elm.length; i++) {
        x = Number(elm.item(i).attributes.getNamedItem("PosX").value);
        ...
        p = new Platform2(x, y, v, r, texture, normal, lightSet);
        gEngine.LayerManager.addToLayer(gEngine.eLayer.eActors, p);
        gEngine.LayerManager.addAsShadowCaster(p);

        allPlatforms2.push(p);
    }
