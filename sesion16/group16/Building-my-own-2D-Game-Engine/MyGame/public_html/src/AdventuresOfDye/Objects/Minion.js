...
Minion.eMinionState = Object.freeze({
    eAlive: 0,
    eDead: 1,
});
...
function Minion(atX, atY, velocity, movementRange, type, texture, normal, lightSet, w, h) {
    ...
    this.eMinionState = Minion.eMinionState.eAlive;  //They are alive until they are not
...
Minion.prototype.changeSprite = function (atX, atY) {
    ...
    switch (this.mType) {
        ...
        case Minion.eMinionType.eChaser:
            this.mMinion.setSpriteSequence(160, 602, 101, 118, 1, 0);
            this.mMinion.setAnimationSpeed(1);
            break;
    }
};

Minion.prototype.playExplosion = function (atX, atY) {
    this.mMinion.setColor([1, 1, 1, 0]);
    this.mMinion.getXform().setPosition(atX, atY);
    this.mMinion.getXform().setSize(this.kWidth, this.kHeight);
    this.mMinion.getXform().setZPos(2);
    this.mMinion.setSpriteSequence(356, 624, 146, 146, 3, 79);
    this.mMinion.setAnimationSpeed(1);
}
...
Minion.prototype.setDeath = function () {
