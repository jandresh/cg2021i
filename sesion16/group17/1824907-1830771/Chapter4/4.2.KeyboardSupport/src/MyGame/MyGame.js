...
MyGame.prototype.update = function () {
  ...
  // WASD Keys to red square
  if (gEngine.Input.isKeyPressed(gEngine.Input.keys.W)) {
        if (redXform.getYPos() > 65) // this is the up-bound of the window
            redXform.setPosition(redXform.getXPos(), 55);
        redXform.incYPosBy(deltaX);
    }

  if (gEngine.Input.isKeyPressed(gEngine.Input.keys.A)) {
        if (redXform.getXPos() < 11) // this is the left-bound of the window
            redXform.setPosition(30, redXform.getYPos());
        redXform.decXPosBy(deltaX);
    }

  if (gEngine.Input.isKeyPressed(gEngine.Input.keys.S)) {
    if (redXform.getYPos() < 55) // this is the bottom-bound of the window
      redXform.setPosition(redXform.getXPos(), 65);
    redXform.decYPosBy(deltaX);
  }

  if (gEngine.Input.isKeyPressed(gEngine.Input.keys.D)) {
    if (redXform.getXPos() > 30) // this is the right-bound of the window
      redXform.setPosition(10, redXform.getYPos());
