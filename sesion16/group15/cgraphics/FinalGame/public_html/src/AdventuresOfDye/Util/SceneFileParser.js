...
SceneFileParser.prototype.parseMovablePlatform = function (texture, normal, lightSet) {
    var elm = this._getElm("MovablePlatform");
    var i, j, x, y, v, r, movP;
    var allMovablePlatforms = [];
    for (i = 0; i < elm.length; i++) {
        ...
        movP = new MovablePlatform(x, y, v, r, texture, normal, lightSet);
        gEngine.LayerManager.addToLayer(gEngine.eLayer.eActors, movP);
        gEngine.LayerManager.addAsShadowCaster(movP);

        allMovablePlatforms.push(movP);
    }

    return allMovablePlatforms;
};
...
SceneFileParser.prototype.parseFloor = function (texture,  normal, lightSet) {
    var elm = this._getElm("Floor");
    var i, x, y, w, h, floor;
    var allFloors = [];
    for (i = 0; i < elm.length; i++) {
        x = Number(elm.item(i).attributes.getNamedItem("PosX").value);
        ...
        floor = new Floor(x, y, w, h, texture, normal, lightSet);
        gEngine.LayerManager.addToLayer(gEngine.eLayer.eActors, floor);
        gEngine.LayerManager.addAsShadowCaster(floor);
...
SceneFileParser.prototype.parseDeadlyFloor = function (texture,  normal, lightSet) {
    var elm = this._getElm("DeadlyFloor");
    var i, x, y, deadlyFloor;
    var allDeadlyFloors = [];
    for (i = 0; i < elm.length; i++) {
        x = Number(elm.item(i).attributes.getNamedItem("PosX").value);
        y = Number(elm.item(i).attributes.getNamedItem("PosY").value);
        ...
        deadlyFloor = new DeadlyFloor(x, y, texture, normal, lightSet);
        gEngine.LayerManager.addToLayer(gEngine.eLayer.eActors, deadlyFloor);
        gEngine.LayerManager.addAsShadowCaster(deadlyFloor);

        allDeadlyFloors.push(deadlyFloor);
    }

    return allDeadlyFloors;
};




SceneFileParser.prototype.parseObjeto = function (texture, normal, lightSet) {
    var elm = this._getElm("Objeto");
    var i, j, x, y, p;
    var allObjects = [];
    for (i = 0; i < elm.length; i++) {
        x = Number(elm.item(i).attributes.getNamedItem("PosX").value);
        ...
        p = new Object(x, y, texture, normal, lightSet);
        gEngine.LayerManager.addToLayer(gEngine.eLayer.eActors, p);
        gEngine.LayerManager.addAsShadowCaster(p);
