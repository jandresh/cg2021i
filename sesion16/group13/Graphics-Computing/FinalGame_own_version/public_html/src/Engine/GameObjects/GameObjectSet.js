 ...
 * @memberOf GameObjectSet
 */
GameObjectSet.prototype.update = function (dyePacks, allParticles, func, camera, winnerFunc, ctx) {
    var i;
    for (i = 0; i < this.mSet.length; i++) {
        if(typeof this.mSet[i].mAllMinions === "object"){
            this.mSet[i].update(dyePacks, allParticles, func, camera, winnerFunc, ctx);
        }
        else {
            this.mSet[i].update();
        }
