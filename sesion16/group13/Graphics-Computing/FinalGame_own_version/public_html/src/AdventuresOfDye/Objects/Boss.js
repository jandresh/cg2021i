function Boss(atX, atY, velocity, movementRange, type, texture0, texture1, texture2,
    ...
    this.kHeight = 8;
    this.kSpeed = 0.02;
    this.kSpawnerTotal = 400; //Tiempo de regeneracion minions
    this.isDead = false; //Saber si esta muerto el boss
    this.amount = 0; // Cantidad de veces que ha sido impactado el boss
    this.heroTouched = 0; // Cantidad de veces que ha sido impactado el heroe
...
Boss.prototype.update = function (dyePacks, allParticles, func, camera, winnerFunc, ctx) {
    GameObject.prototype.update.call(this);
    var i;
    for (i = 0; i < this.mAllMinions.length; i++) {
        this.mAllMinions[i].update(this.mHeroRef, dyePacks, allParticles, func);
    }
    ...
    if(this.heroTouched >= 2) {
        winnerFunc.call(ctx, false); 
    } // En caso de que haya perdido
    
    // Las colisiones entre el disparo y el boss
    ...
    for (i=0; i<dyePacks.size(); i++) {
        ...
        if (obj.pixelTouches(this, collisionPt)) {
            ...
            if(this.amount >= 20){
                this.isDead = true;
                winnerFunc.call(ctx, true);            
            }
        }
    }
    
    //Si el minion ha chocado con el heroe o se le ha disparado mas de 6 veces eliminamos el minion
    this.mAllMinions = this.mAllMinions.filter((minion)=> {
        if(minion.amount < 6 && !minion.isTouched) {
            return minion;
        }
        if(minion.isTouched){
            this.heroTouched++;
